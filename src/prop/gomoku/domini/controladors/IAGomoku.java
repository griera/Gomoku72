...
public class IAGomoku extends InteligenciaArtificial
{
	 ...
	 * conseguir l'objectiu final (muntar una línia de cinc fitxes seguides d'un mateix color). Una línia de cinc val
	 * la màxima puntuació (Integer.MAX:VALUE o Integer.MIN_VALUE en funció dels interessos del jugador controlat per 
	 ...
	 * 
	 * Aquest càlcul s'ha de realitzar per cada jugador que disputa la partida en el tauler donat, és a dir, tindrem
	 * la suma total de totes les estructures potencialment guanyadores per al jugador amb fitxes negres, i la suma
	 ...
	 * tingui aquest jugador, haurem de restar la suma de total de les estructures potencialment guanyadores
	 * pertanyents a les fitxes del seu color menys la suma total homòloga de les fitxes del jugador oponent.
	 * 
	 ...
	 *  
	 * - Estructura complexa: Fusió de dues estructures simples formades a partir d'una fitxa central, que representa
	 *   el punt de connexió de les dues estructures simples.
	 *    
	...
	public int funcioAvaluacio( Tauler tauler, EstatPartida estat_moviment, int profunditat, EstatCasella fitxa_jugador )
	{
		 ...
		 * puntuació màxima.
		 * 
		 * Si la paritat és senar, voldrà dir que hem arribat a un node Max segons l'algorisme MiniMax, és a dir, que
		 * el jugador controlat per aquesta intel·ligència artificial haurà guanyat la partida.
		 * 
		...
		else
		{
			 ...
			 * col·lumna del array al qual fan referència):
			 *  
			 ...
			 * tauler donat.
			 * La primera fila conté el recompte de totes les estructures potencialment guanyadores per a les fitxes
			 * de color negre, i la segona fila conté el mateix recompte però per a les fitxes de color blanc.
			 * L'ordre en que es guarden és el mateix que l'ordre de les seves puntuacions en el array puntuacions.
			 */ 
			...
			for ( int fila = 0; fila < mida; ++fila )
			{
				for ( int columna = 0; columna < mida; ++columna )
				{
					...
					if ( tauler.getEstatCasella( fila, columna ) != EstatCasella.BUIDA )
					{
						 ...
						 * ho fa si la fitxa actual és de color blanc.
						 * L'ordre en que es guarden les estructuresés el mateix que l'ordre de les seves puntuacions
						 * en el array puntuacions.
						 */ 
						int[][] opcions_linia = new int[2][14];
						
						computaFila( (TaulerGomoku) tauler, estat, fila, columna, opcions_linia );
						computaColumna( (TaulerGomoku) tauler, estat, fila, columna, opcions_linia );
						computaDiagonalNoSe( (TaulerGomoku) tauler, estat, fila, columna, opcions_linia );
						computaDiagonalNeSo( (TaulerGomoku) tauler, estat, fila, columna, opcions_linia );
						fusionaEstructures( estat, opcions_linia );
						...
						/* 
						 * Si no hi ha encara cap estructura guanyadora (cap guanyador de la partida), aleshores s'ha
						 * de calcular la puntuació acumulada per les estructures potencialment guanyadores formades al
						 * voltant de la fitxa actual (càlcul incremental per cadascun dels jugadors), i sumar-les
						 * a les variables que acumulen la puntuació total donat un tauler (avaluacio_negre i
						 * avaluacio_blanc)
						 */
			 ...
			 * segons l'estat del tauler donat. Si és aquest cas, cal donar la puntuació màxima en funció del
			 * jugador controlat per aquesta intel·ligència artificial. Retornarem la màxima puntuació 
			 * (Integer.MAX_VALUE) si és ell el que s'ha assegurat la victòria abans d'hora, i retornarem 
			 * (Integer.MIN_VALUE) si és el seu oponent qui ja té la victòria assegurada.
			 * 
			 * Aquesta selecció es basa en les estructures potencialment guanyadores que tenen cadascun dels jugadors
			 * en el tauler indicat.
			 */
			 ...
			 * Ara només cal retornar els resultats obtinguts en funció del color de les fitxes del jugador controlat 
			 * per aquesta intel·ligència artificial. Si té fitxes negres cal retornar la resta entre: 
			 * avaluació_negre - avaluacio_blanc 
			 * 
			 * Si, en canvi, si té les fitxes blanques cal retornar: 
			 * avaluacio_blanc - avaluacio_negre
			 */
	 ...
	 * </em>) en estructures potencialment guanyadores complexes, actualitzant d'aquesta manera el recompte acumulat
	 * en el mateix array.
	 * <p>
	 ...
	 */
	private void fusionaEstructures( EstatCasella estat, int[][] opcions_linia )
	{
	 ...
	 * <p>
	 * Un cop analitzada de quina estructura es tracta, aleshores es guarda dins del array <em>opcions_linia</em> en
	 * la seva posició corresponent, segons el color de la fitxa a partir de la qual s'ha llegit l'estructura
	 * potencialment guanyadora i segons quina puntuació tingui associada.
	 *  
	 ...
	 * @param columna Índex de la col·lumna on es troba situada la fitxa a avaluar dins de <em>tauler</em>.
	...
	private void computaFila( TaulerGomoku tauler, EstatCasella estat, int fila, int columna, int[][] opcions_linia )
	{
		...
		{
			classificaEstructura( estat, opcions_linia, oberta, semioberta, linia_amb_forat, fitxes_seguides );
		}
	}

	/**
	 * Compta el nombre d'estructures potencialment guanyadores que es troben recorrent la col·lumna (en els dos sentits 
	 * possibles) on està situada la fitxa actual sobre <em>tauler</em> (en la posició (<em>fila</em>, <em>columna</em>
	 * )).
	 * 
	 * @param tauler Objecte de la classe <code>TaulerGomoku</code> sobre el qual realitzar el recompte.
	 ...
	 * @param columna Índex de la col·lumna a on es vol efectuar el moviment adjacent.
	 * @param mida Mida del tauler a on es juga la partida al Gomoku (ha de ser un tauler quadrat, amb el mateix
	 *        nombre de files que de col·lumnes).
	 ...
	 *         primer element representa l'índex de la fila i el segon l'índex de la col·lumna de <em>tauler</em>
	 */
