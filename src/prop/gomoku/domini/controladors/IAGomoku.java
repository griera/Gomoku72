...
public class IAGomoku extends InteligenciaArtificial
{
	...
	public int funcioAvaluacio(Tauler tauler, EstatPartida estat_partida, int profunditat, EstatCasella fitxa_jugador ) {
	    if (estat_partida == EstatPartida.EMPAT) return 0;
	    else if (estat_partida != EstatPartida.NO_FINALITZADA) { // suposant que mai cridarem al mètode per a un estat invalid!
	        if (profunditat % 2 == 1) return Integer.MAX_VALUE;
	        ...
	        for (int x = 0; x < m; ++x) {
	            for (int y = 0; y < m; ++y) {
	                ...
	                if (tauler.getEstatCasella(x, y) != EstatCasella.BUIT) {
	                    ...
	                    // Comprovació fila
	                    for (int k = y + 1; k < m && (estat == tauler.getEstatCasella( x, k ) || tauler.getEstatCasella( x, k ) == EstatCasella.BUIT ); ++k) {
	                        ++fitxes_seguides;
	                    ...
	                    for (int k = y - 1; k >= 0 && (estat == tauler.getEstatCasella( x, k ) || tauler.getEstatCasella( x, k ) == EstatCasella.BUIT ); ++k) {
	                        ++fitxes_seguides;
	                        if (fitxes_seguides >= 5) {
	                            switch (estat) {
	                                case JUGADOR_A:
	        ...
	        if (fitxa_jugador == EstatCasella.JUGADOR_A) return avaluacio;
	        return -avaluacio;
	    }
	}
